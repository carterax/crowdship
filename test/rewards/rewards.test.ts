...
contract('CampaignRewards', function([
  campaignOwner,
  root,
  factoryWallet,
  addr1,
  ...
  it('should create a reward', async function() {
    ...
    expect((await this.rewardsInstance.rewards(0)).value).to.be.bignumber.equal(
      new BN('100')
    );
    expect(
      (await this.rewardsInstance.rewards(0)).deliveryDate
    ).to.be.bignumber.equal(new BN('86400'));
    expect((await this.rewardsInstance.rewards(0)).stock).to.be.bignumber.equal(
      new BN('10')
    );
